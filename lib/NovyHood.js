...
class NovyHood extends EventEmitter {
  ...
  constructor({ homey, uuid, address, localName, log, error }) {
    ...
    // Behaviour overrides (device settings). The hood natively restores its
    // last-used state on power-on; these enforce house rules instead.
    this.skipRunOut = false;
    this.startMinimum = false;
    this.lightWarmFull = false;

    // Explicit user commands (tile/flow) must not be "corrected" by the
    // enforcement watchers moments later.
    this.lastExplicitFanAt = 0;
    this.lastExplicitLightAt = 0;
  ...
  async setFanSpeed(pct) {
    this._cancelAutoOff();
    this.lastExplicitFanAt = Date.now();
  ...
  async setLight(on, { brightness, colortemp } = {}) {
    ...
    if (brightness !== undefined || colortemp !== undefined) {
      this.lastExplicitLightAt = Date.now();
    }
  ...
  /** Request a fresh main-status packet (state lands via the usual parser). */
  async refreshStatus() {
    await this._send(CMD.STATUS, 0);
  }

  /** Light on in the warmest colour (ambient preset) at full brightness. */
  async setLightWarmFull() {
    await this._send(CMD.LIGHT_ON_AMBI, 0);
    await this._send(CMD.LIGHT_BRIGHTNESS, 1, 255);
    await this._send(CMD.LIGHT_COLORTEMP, 1, 255);
    await this._send(CMD.STATUS, 0);
  }

  /** Direct off: fan and light, no run-out. Idempotent. */
  async turnAllOff() {
    this._cancelAutoOff();
    await this._send(CMD.FAN_STATE, 1, 0);
    await this._send(CMD.LIGHT_OFF, 0);
    await this._send(CMD.STATUS, 0);
  }
